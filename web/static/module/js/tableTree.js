layui.define(["laydate", "laytpl", "table", "layer", "tableEdit", "form"], function (exports) {
    ...
    TableTree.prototype = {
        ...
        render: function (options, isFirstRender) {
            ...
            if (isFirstRender) {
                ...
                /*
                添加对行操作图标  add,update,remove,sort的自定义
                sxj add 2021年3月5日
                 */
                //默认全部不显示
                treeConfig.rowToolIcoEnable = {add: false, update: false, remove: false, sort: false};
                if (treeConfig.rowToolIcon !== undefined) {
                    //处理传入数组的情况
                    if (Array.isArray(treeConfig.rowToolIcon)) {
                        treeConfig.rowToolIcoEnable.add = treeConfig.rowToolIcon.includes("add");
                        treeConfig.rowToolIcoEnable.update = treeConfig.rowToolIcon.includes("update");
                        treeConfig.rowToolIcoEnable.remove = treeConfig.rowToolIcon.includes("remove");
                        treeConfig.rowToolIcoEnable.sort = treeConfig.rowToolIcon.includes("sort");
                    }
                    //处理为true的情况
                    if (treeConfig.rowToolIcon === true) {
                        treeConfig.rowToolIcoEnable = {add: true, update: true, remove: true, sort: true};
                    }
                }
                //自定义END
        ...
        , doInitTree: function (arr, field, tr, lvl) {
            ...
            /*
            update 2021年3月14日 sxj
            由于原插件是通过以父节点的tr为目标。将子节点插入到父节点之后（after()方法），
            导致先插入的元素在后，与后端返回数据顺序不一致
            所有，对lvl大于0的数据先倒排排一遍
             */
            if(lvl>0 && Array.isArray(arr))arr.reverse();
            _layui.each(arr, function (key, item) {
                if (lvl === 0) {
                    ...
                    // var text = item[that.config.treeConfig.showField];//获取显示字段的值 无法获取模板解析后的值
                    var text = that.parseTempletData(item, field); //按模板进行解析
                    text = configs.isEmpty(text) ? "" : text;
                     !span[0] && (_div.text(''), _div.prepend('<div class="layui-tableTree-div"><span class="layui-tableEdit-span">' + text + '</span></div>'));
                    var isLeaf=item.treeList === undefined || item.treeList===[];
                    if (!td.find('i.layui-tableEdit-edge')[0]) that.addIcon(tr, td, lvl,isLeaf);
                } else {
         ...
         * @param lvl 当前tr等级
         */
        , addIcon: function (tr, td, lvl,isLeaf) {
            ...
            if (!isLeaf){//如果是叶子节点就不添加图标，非叶子节点添加图标
                div.append(icon);
            }
            $(td).append(_div);
            var add = $('<i class="layui-icon layui-icon-add-1 layui-tableEdit-edit"></i>');
            if (this.config.treeConfig.rowToolIcoEnable.add)_div.append(add);//2021年3月6日 sxj 添加了判断，只有在参数中配置了才会添加图标
            var update = $('<i class="layui-icon layui-icon-edit layui-tableEdit-edit"></i>');
            if (this.config.treeConfig.rowToolIcoEnable.update)_div.append(update);//2021年3月6日 sxj 添加了判断，只有在参数中配置了才会添加图标
            var remove = $('<i class="layui-icon layui-icon-delete layui-tableEdit-edit"></i>');
            if (this.config.treeConfig.rowToolIcoEnable.remove)_div.append(remove);//2021年3月6日 sxj 添加了判断，只有在参数中配置了才会添加图标
            var sort = $('<i class="layui-icon layui-icon-return layui-tableEdit-edit asc" style="font-size: 16px"></i>');
            if (this.config.treeConfig.rowToolIcoEnable.sort)_div.append(sort);//2021年3月6日 sxj 添加了判断，只有在参数中配置了才会添加图标
            _div.hide();
            sort.css('transform', 'rotate(90deg)');
        ...
        , events: function () {
            ...
            tableBody.on('click', 'i.layui-tableEdit-edit', function (e) {
                ...
                //删除
                if (isDelete) {
                    layer.confirm('<div style="color: #ff0000;text-align: center;">确定删除吗？</div>', function (index) {
                        var thisObj = {
            ...
            table.on('radio(' + filter + ')', function (obj) {
                var thisInput = this
                    , treeData = configs.tableTreeCache[tableTreeid];
                var tr = $(thisInput).parents('tr'), lvl = tr.data('lvl');
                // lvl = lvl ? lvl : 0;
                ...
                var _treeid = tr.attr("tree-id");
                //去除其他行的选择
                var allTr=tr.parents('tbody').get(0).childNodes;
                for (var trIndex=0;trIndex<allTr.length;trIndex++){
                    var theTr=$(allTr[trIndex]);
                    var iconDiv=$(theTr.find(".layui-unselect.layui-form-radio")[0]);
                    var icon=$(theTr.find(".layui-anim.layui-icon")[0]);
                    var radio=$(theTr.find('input[type="radio"]')[0]);
                    if (_treeid===theTr.attr("tree-id")){//当前选中行
                        theTr.addClass("layui-table-click");
                        iconDiv.addClass("layui-form-radioed layui-form-checked");
                        icon.addClass("layui-anim-scaleSpring");
                        icon.html("&#xe643;");
                        radio.prop('checked', true);
                    }else{//其他行
                        theTr.removeClass("layui-table-click");
                        iconDiv.removeClass("layui-form-radioed layui-form-checked");
                        icon.removeClass("layui-anim-scaleSpring");
                        icon.html("&#xe63f;");
                ...
                //所以在此直接以此方式获取 保证取到相对应节点数据
                obj.data = that.getDataByTreeId(treeData, _treeid);
                active.callbackFn.call(this, 'radio(' + filter + ')', obj);
            });
                ...
                var td = $(this).parents('td')
                    ...
                    value: null, data: data, field: field, async: function (asyncTree) {
                        ...
                        while (count < asyncTree.length) {
                            data.treeList.push(asyncTree[count]);
                            count++;
                        }
                        //图标改回来
                        that.openTreeNode(tr);
        ...
        getCheckedTreeNodeDataWithParent: function () {
            var that = this;
            var tableBody = $(this.config.elem).next().find('div.layui-table-body');
            var tableTreeid = $(that.config.elem).attr('id');
            var treeData = configs.tableTreeCache[tableTreeid];
            ...
            checkedElem.each(function () {
                ...
                if (lvl>0){
                    ...
                    var parentTreeid = parentNode.attr('tree-id');
                    parentData=that.getDataByTreeId(treeData,parentTreeid)
                }
                dataSet.add({data:data,pData:parentData});
            });
            return Array.from(dataSet);
        }
        /**
            ...
            tr.children('td').each(function () {
                ...
                if (field + "" === '0') {
                    ...
                    if (field === that.config.treeConfig.showField) {
                        ...
                        var isLeaf=data.treeList === undefined || data.treeList===[];
                        that.addIcon(newTr, td, lvl,isLeaf);
                    } else {
    ...
    var active = {
        on: function (event, callback) {
            var filter = event.match(/\((.*)\)$/),
                eventName = (filter ? (event.replace(filter[0], '') + '_' + filter[1]) : event);
        ...
        render: function (options) {
            ...
            return {
                ...
                getCheckedTreeNodeDataWithParent: function () {
                    return tableTree.getCheckedTreeNodeDataWithParent();
                },
                reload: function (options) {
